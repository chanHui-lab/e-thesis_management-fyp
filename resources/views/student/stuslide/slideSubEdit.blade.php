...
<main>
    ...
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Edit Presentation Slide</h3>
        </div>
        {{-- <form method="POST" action="" enctype="multipart/form-data"> --}}

        <form method="POST" action="{{ route('slideSubmission.update', ['slideSubmissionId' => $getRecord->id]) }}" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                <div class="form-group">
                    <label for="slide_title"  class = "labelling">Title:</label>
                    <input type="text" id="slide_title" name="slide_title" class = "form-control" placeholder="Enter Title..." value="{{ $getRecord->slide_title }}"><br>

                </div>
                <div class="form-group">
                    <label for="slide_description" class = "labelling">Description:</label>
                    <textarea id="slide_description" class="form-control" style="height: 150px" name="slide_description" rows="4" cols="50" placeholder="slide_description">{{ $getRecord->slide_description }}</textarea><br>
                </div>

                <p>Current File:</p>
                @php
                    $files = json_decode($getRecord->slide_file, true);
                @endphp
                ...
                <label for="slide_file" class = "labelling">Upload Proposal Files:</label><br>
...
<script>
    $(document).ready(function () {
        ...
        $('#removeFileBtn').click(function () {
            ...
            $.ajax({
                url: '{{ route("slideSubmission.remove-file", $getRecord->id) }}',
                type: 'DELETE',
                headers: {
